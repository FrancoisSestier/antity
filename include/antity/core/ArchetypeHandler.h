    ...
    class ArchetypeHandler {
       public:
        ArchetypeHandler(EntityIndex* entityIndex, ComponentMap* componentMap)
            : entityIndex(entityIndex), componentMap(componentMap) {}

        template <typename C, typename... Args>
        void EmplaceComponent(Archetype* archetype, ComponentBase* component,
                              size_t componentIndex, Args... args) {
            archetypeAllocator.AutoAllocate(archetype, componentIndex,
                                            component);
            C* newComponent
        ...
        void MoveLastComponent(Archetype* archetype, size_t toIndex,
                               size_t componentIndex,
                               ComponentBase* component) {
            MoveComponent(archetype, archetype, archetype->entities.size() - 1,
                          toIndex, componentIndex, componentIndex, component);
        }

        void MoveComponent(Archetype* from, Archetype* to, size_t fromIndex,
                           size_t toIndex, size_t oldComponentIndex,
                           size_t newComponentIndex, ComponentBase* component) {
            if (from != to) {
                archetypeAllocator.AutoAllocate(to, newComponentIndex,
                                                component);
            }
            size_t componentSize = component->GetSize();
            component->MoveData(&from->byteArrays[oldComponentIndex]
        ...
        void EraseComponent(Archetype* archetype, const size_t index,
                            size_t ComponentIndex, ComponentBase* component) {
            component->DestroyData(
                &archetype->byteArrays[ComponentIndex]
                     .componentData[index * component->GetSize()]);
        ...
        void CleanArchetypeComponentArrays(Archetype* archetype) {
            for (int j = 0; j < archetype->byteArrays.size(); j++) {
                auto component
                    = componentMap->at(archetype->archetypeId[j]).get();
                for (int i = 0; i < archetype->entities.size(); i++) {
                    component->DestroyData(
                        &archetype->byteArrays[j]
                             .componentData[i * component->GetSize()]);
                }
        ...
        C& GetComponent(Archetype* archetype, size_t index) {
            const size_t componentIndex
                = std::ranges::find(archetype->archetypeId.begin(),
                                    archetype->archetypeId.end(),
                                    TypeIdGenerator::GetTypeID<C>())
                  - archetype->archetypeId.begin();
            return *std::launder(
