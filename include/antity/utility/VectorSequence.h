...
namespace ant {
    ...
    class VectorSequence {
       ...
       public:
        ...
        iterator end() {
            return iterator{ends.back(), &begins, &ends, ends.size() - 1};
        }
    ...
    struct VectorZipper {
        ...
        using underlying_iterator = std::tuple<
            MultiIterator<typename std::vector<Args>::iterator>...>;
        using iterator = ZipIterator<
            std::tuple<MultiIterator<typename std::vector<Args>::iterator>...>,
            Args...>;

        VectorZipper(underlying_iterator begins, underlying_iterator ends)
    ...
    VectorSequence<T> makeVectorSequence(Args&&... args) {
        return VectorSequence<T>(
            std::vector<std::vector<T>*>{&std::forward<Args>(args)...});
    }
}  // namespace ant
